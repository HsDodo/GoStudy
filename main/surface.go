...
func corner(i, j int) (float64, float64) {
	x := xyrange * (float64(i)/cells - 0.5) //单元格(i,j)的顶点坐标
	y := xyrange * (float64(j)/cells - 0.5) //单元格(i,j)的顶点坐标
	z := f(x, y)                            //计算曲面高度
	//将(x,y,z)等角投射到二维SVG绘图平面上，坐标是(sx,sy)
	sx := width/2 + (x-y)*cos30*xyscale             //等角投影 x-y是因为x轴是倾斜的 cos30*xyscale是因为x轴是倾斜的、
	sy := height/2 + (x+y)*sin30*xyscale - z*zscale //等角投影 这里的sx, sy 是相对于画布的坐标，
	return sx, sy
}
